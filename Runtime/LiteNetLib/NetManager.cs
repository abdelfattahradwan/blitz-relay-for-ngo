...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public class NetManager : LiteNetManager, IEnumerable<NetPeer>
	{
		...
		public byte ChannelsCount
		{
			...
			set
			{
				if (value < 1 || value > 64) throw new ArgumentException("Channels count must be between 1 and 64");
		...
		public void GetPeers(List<NetPeer> peers, ConnectionState peerState)
		{
			...
			{
				if ((netPeer.ConnectionState & peerState) != 0) peers.Add((NetPeer)netPeer);
			}
		...
		/// </summary>
		/// <param name="peers">List that will contain result</param>
		public void GetConnectedPeers(List<NetPeer> peers) => GetPeers(peers, ConnectionState.Connected);

		public NetManager(INetEventListener listener, PacketLayerBase extraPacketLayer = null) : base(null, extraPacketLayer) => _netEventListener = listener;

		/// <summary>
		/// Create the requests for NTP server
		/// </summary>
		/// <param name="endPoint">NTP Server address.</param>
		public void CreateNtpRequest(IPEndPoint endPoint) => _ntpRequests.TryAdd(endPoint, new NtpRequest(endPoint));
		...
		//connect to
		protected override LiteNetPeer CreateOutgoingPeer(IPEndPoint remoteEndPoint, int id, byte connectNum, ReadOnlySpan<byte> connectData) => new NetPeer(this, remoteEndPoint, id, connectNum, connectData);

		//accept
		protected override LiteNetPeer CreateIncomingPeer(LiteConnectionRequest request, int id) => new NetPeer(this, request, id);

		//reject
		protected override LiteNetPeer CreateRejectPeer(IPEndPoint remoteEndPoint, int id) => new NetPeer(this, remoteEndPoint, id);

		//connection request when you use Accept for getting NetPeer instead of LiteNetPeer
		protected override LiteConnectionRequest CreateConnectionRequest(IPEndPoint remoteEndPoint, NetConnectRequestPacket requestPacket) => new ConnectionRequest(remoteEndPoint, requestPacket, this);
		...
		protected override void ProcessEvent(NetEvent evt)
		{
			...
			switch (evt.Type)
			{
				...
				case NetEvent.EType.PeerAddressChanged:
					...
					if (previousAddress != null) _netEventListener.OnPeerAddressChanged(netPeer, previousAddress);
		...
		{
			if (_ntpRequests.IsEmpty) return;
			...
			foreach (var ntpRequest in _ntpRequests)
			{
				...
				{
					if (requestsToRemove == null) requestsToRemove = new List<IPEndPoint>();
		...
		public void SendToAll(NetDataWriter writer, byte channelNumber, DeliveryMethod options) => SendToAll(writer.Data, 0, writer.Length, channelNumber, options);
		...
		/// <param name="channelNumber">Number of channel (from 0 to channelsCount - 1)</param>
		/// <param name="options">Send options (reliable, unreliable, etc.)</param>
		public void SendToAll(byte[] data, byte channelNumber, DeliveryMethod options) => SendToAll(data, 0, data.Length, channelNumber, options);
		...
		public void SendToAll(NetDataWriter writer, byte channelNumber, DeliveryMethod options, LiteNetPeer excludePeer) => SendToAll(writer.Data, 0, writer.Length, channelNumber, options, excludePeer);
		...
		/// <param name="options">Send options (reliable, unreliable, etc.)</param>
		/// <param name="excludePeer">Excluded peer</param>
		public void SendToAll(byte[] data, byte channelNumber, DeliveryMethod options, LiteNetPeer excludePeer) => SendToAll(data, 0, data.Length, channelNumber, options, excludePeer);
		...
		public void SendToAll(byte[] data, int start, int length, byte channelNumber, DeliveryMethod options, LiteNetPeer excludePeer)
		{
			try
			{
				...
				for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer)
				{
					if (netPeer != excludePeer) ((NetPeer)netPeer).Send(data, start, length, channelNumber, options);
				}
			}
			finally
			{
		...
		public void SendToAll(byte[] data, int start, int length, byte channelNumber, DeliveryMethod options)
		{
			...
			{
				_peersLock.EnterReadLock();
				for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer) ((NetPeer)netPeer).Send(data, start, length, channelNumber, options);
			}
			finally
			{
				_peersLock.ExitReadLock();
		...
		/// <param name="channelNumber">Number of channel (from 0 to channelsCount - 1)</param>
		/// <param name="options">Send options (reliable, unreliable, etc.)</param>
		public void SendToAll(ReadOnlySpan<byte> data, byte channelNumber, DeliveryMethod options) => SendToAll(data, channelNumber, options, null);
		...
		public void SendToAll(ReadOnlySpan<byte> data, byte channelNumber, DeliveryMethod options, LiteNetPeer excludePeer)
		{
			try
			{
				...
				for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer)
				{
					if (netPeer != excludePeer) ((NetPeer)netPeer).Send(data, channelNumber, options);
				}
			}
			finally
			{
		...
		public new NetPeer Connect(string address, int port, string key) => Connect(address, port, NetDataWriter.FromString(key));
		...
		/// <returns>New NetPeer if new connection, Old NetPeer if already connected, null peer if there is ConnectionRequest awaiting</returns>
		/// <exception cref="InvalidOperationException">Manager is not running. Call <see cref="LiteNetManager.Start()"/></exception>
		public new NetPeer Connect(string address, int port, NetDataWriter connectionData) => (NetPeer)base.Connect(address, port, connectionData);
		...
		/// <returns>New NetPeer if new connection, Old NetPeer if already connected, null peer if there is ConnectionRequest awaiting</returns>
		/// <exception cref="InvalidOperationException">Manager is not running. Call <see cref="LiteNetManager.Start()"/></exception>
		public new NetPeer Connect(IPEndPoint target, string key) => (NetPeer)base.Connect(target, key);
		...
		/// <returns>New NetPeer if new connection, Old NetPeer if already connected, null peer if there is ConnectionRequest awaiting</returns>
		/// <exception cref="InvalidOperationException">Manager is not running. Call <see cref="LiteNetManager.Start()"/></exception>
		public new NetPeer Connect(IPEndPoint target, NetDataWriter connectionData) => (NetPeer)base.Connect(target, connectionData);
		...
		/// <returns>New NetPeer if new connection, Old NetPeer if already connected, null peer if there is ConnectionRequest awaiting</returns>
		/// <exception cref="InvalidOperationException">Manager is not running. Call <see cref="LiteNetManager.Start()"/></exception>
		public new NetPeer Connect(IPEndPoint target, ReadOnlySpan<byte> connectionData) => (NetPeer)base.Connect(target, connectionData);

		public new NetPeerEnumerator<NetPeer> GetEnumerator() => new NetPeerEnumerator<NetPeer>((NetPeer)_headPeer);

		IEnumerator<NetPeer> IEnumerable<NetPeer>.GetEnumerator() => new NetPeerEnumerator<NetPeer>((NetPeer)_headPeer);
	}
}
