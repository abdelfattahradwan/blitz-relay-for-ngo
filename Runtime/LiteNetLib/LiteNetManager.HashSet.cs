	...
	public partial class LiteNetManager
	{
		...
		protected bool ContainsPeer(LiteNetPeer item)
		{
			...
			if (_buckets != null)
			{
				...
				{
					if (_slots[i].HashCode == hashCode && _slots[i].Value.Equals(item)) return true;
				}
			}
		...
		private void AddPeer(LiteNetPeer peer)
		{
			...
			if (peer.Id >= _peersArray.Length)
			{
				...
				while (peer.Id >= newSize) newSize *= 2;
		...
		{
			if (enableWriteLock) _peersLock.EnterWriteLock();

			if (!RemovePeerFromSet(peer))
			{
				if (enableWriteLock) _peersLock.ExitWriteLock();

				return;
			}

			if (peer == _headPeer) _headPeer = peer.NextPeer;

			if (peer.PrevPeer != null) peer.PrevPeer.NextPeer = peer.NextPeer;
			if (peer.NextPeer != null) peer.NextPeer.PrevPeer = peer.PrevPeer;
			peer.PrevPeer = null;
		...
		{
			if (_buckets == null || peer == null) return false;
			...
			for (int i = _buckets[bucket] - 1; i >= 0; last = i, i = _slots[i].Next)
			{
				...
				{
					if (last < 0) _buckets[bucket] = _slots[i].Next + 1;
					else _slots[last].Next = _slots[i].Next;
		...
		protected bool AddPeerToSet(LiteNetPeer value)
		{
			...
			for (int i = _buckets[hashCode % _buckets.Length] - 1; i >= 0; i = _slots[i].Next)
			{
				if (_slots[i].HashCode == hashCode && _slots[i].Value.Equals(value)) return false;
			}
