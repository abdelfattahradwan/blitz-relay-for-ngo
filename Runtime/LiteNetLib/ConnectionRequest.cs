...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public class LiteConnectionRequest
	{
		...
		{
			//old request
			if (connectRequest.ConnectionTime < InternalPacket.ConnectionTime) return;

			if (connectRequest.ConnectionTime == InternalPacket.ConnectionTime &&
				connectRequest.ConnectionNumber == InternalPacket.ConnectionNumber) return;

			InternalPacket = connectRequest;
		}

		private bool TryActivate() => Interlocked.CompareExchange(ref _used, 1, 0) == 0;
		...
		public LiteNetPeer AcceptIfKey(string key)
		{
			...
			try
			{
				if (Data.GetString() == key) Result = ConnectionRequestResult.Accept;
			}
			catch
			{
			...
			if (Result == ConnectionRequestResult.Accept) return _listener.OnConnectionSolved(this, null, 0, 0);
		...
		public void Reject(byte[] rejectData, int start, int length) => Reject(rejectData, start, length, false);
		...
		/// <param name="start">Offset in the <paramref name="rejectData"/> array.</param>
		/// <param name="length">Length of the data to be sent.</param>
		public void RejectForce(byte[] rejectData, int start, int length) => Reject(rejectData, start, length, true);
		...
		/// </summary>
		/// <param name="rejectData">Data to send with the rejection.</param>
		public void RejectForce(byte[] rejectData) => Reject(rejectData, 0, rejectData.Length, true);
		...
		public void RejectForce(NetDataWriter rejectData) => Reject(rejectData.Data, 0, rejectData.Length, true);
		...
		/// </summary>
		/// <param name="rejectData">Data to send with the rejection.</param>
		public void Reject(byte[] rejectData) => Reject(rejectData, 0, rejectData.Length, false);

		/// <summary>
		/// Rejects the connection reliably using data from a <see cref="NetDataWriter"/>.
		/// </summary>
		/// <param name="rejectData">Writer containing the data to send.</param>
		public void Reject(NetDataWriter rejectData) => Reject(rejectData.Data, 0, rejectData.Length, false);
	}
