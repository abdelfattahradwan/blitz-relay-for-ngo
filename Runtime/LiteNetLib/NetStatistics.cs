...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public sealed class NetStatistics
	{
		...
		public void AddBytesSent(long bytesSent) => Interlocked.Add(ref _bytesSent, bytesSent);

		/// <summary>
		/// Adds a specific amount to the total bytes received.
		/// </summary>
		/// <param name="bytesReceived">Number of bytes to add.</param>
		public void AddBytesReceived(long bytesReceived) => Interlocked.Add(ref _bytesReceived, bytesReceived);

		/// <summary>
		/// Increments the count of lost packets by one.
		/// </summary>
		public void IncrementPacketLoss() => Interlocked.Increment(ref _packetLoss);

		/// <summary>
		/// Adds a specific amount to the total packet loss count.
		/// </summary>
		/// <param name="packetLoss">Number of lost packets to add.</param>
		public void AddPacketLoss(long packetLoss) => Interlocked.Add(ref _packetLoss, packetLoss);
