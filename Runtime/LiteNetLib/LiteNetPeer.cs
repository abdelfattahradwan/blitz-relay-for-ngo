...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public class LiteNetPeer : IPEndPoint
	{
		...
		public override SocketAddress Serialize() => _cachedSocketAddr;
		...
		internal LiteNetPeer(LiteNetManager netManager, IPEndPoint remoteEndPoint, int id) : base(remoteEndPoint.Address, remoteEndPoint.Port)
		{
			...
			if (NetManager.UseNativeSockets)
			{
				...
				for (int i = 0; i < _cachedSocketAddr.Size; i++) NativeAddress[i] = _cachedSocketAddr[i];
			}
		...
		{
			if (_connectionState != ConnectionState.EndPointChange) return;
		...
		public int GetPacketsCountInReliableQueue(bool ordered) => (ordered ? _reliableChannel : _reliableUnorderedChannel)?.PacketsInQueue ?? 0;
		...
		public void SendPooledPacket(PooledPacket packet, int userDataSize)
		{
			...
			else
			{
				...
				{
					if (_unreliablePendingCount == _unreliableChannel.Length) Array.Resize(ref _unreliableChannel, _unreliablePendingCount * 2);
		...
		{
			if (_connectionState != ConnectionState.Outgoing) return false;
		...
		public int GetMaxSinglePacketSize(DeliveryMethod options) => _mtu - NetPacket.GetHeaderSize(options == DeliveryMethod.Unreliable ? PacketProperty.Unreliable : PacketProperty.Channeled);
		...
		{
			if (deliveryMethod != DeliveryMethod.ReliableOrdered && deliveryMethod != DeliveryMethod.ReliableUnordered) throw new ArgumentException("Delivery event will work only for ReliableOrdered/Unordered packets");
		...
		/// </exception>
		public void Send(byte[] data, DeliveryMethod deliveryMethod) => SendInternal(new ReadOnlySpan<byte>(data, 0, data.Length), 0, deliveryMethod, null);
		...
		///     Fragment count exceeded ushort.MaxValue<para/>
		/// </exception>
		public void Send(NetDataWriter dataWriter, DeliveryMethod deliveryMethod) => SendInternal(dataWriter.AsReadOnlySpan(), 0, deliveryMethod, null);
		...
		///     Fragment count exceeded ushort.MaxValue<para/>
		/// </exception>
		public void Send(byte[] data, int start, int length, DeliveryMethod options) => SendInternal(new ReadOnlySpan<byte>(data, start, length), 0, options, null);
		...
		///     Fragment count exceeded ushort.MaxValue<para/>
		/// </exception>
		public void Send(ReadOnlySpan<byte> data, DeliveryMethod deliveryMethod) => SendInternal(data, 0, deliveryMethod, null);
		...
		{
			if (_connectionState != ConnectionState.Connected || channelNumber >= ChannelsCount) return;
			...
			if (length + headerSize > mtu)
			{
				...
				if (deliveryMethod != DeliveryMethod.ReliableOrdered && deliveryMethod != DeliveryMethod.ReliableUnordered) throw new TooBigPacketException("Unreliable or ReliableSequenced packet size exceeded maximum of " + (mtu - headerSize) + " bytes, Check allowed size by GetMaxSinglePacketSize()");

				int packetFullSize = mtu - headerSize;
				int packetDataSize = packetFullSize - NetConstants.FragmentHeaderSize;
				int totalPackets = length / packetDataSize + (length % packetDataSize == 0 ? 0 : 1);

				if (totalPackets > NetManager.MaxFragmentsCount) throw new TooBigPacketException("Data was split in " + totalPackets + " fragments, which exceeds " + NetManager.MaxFragmentsCount);
		...
		public void Disconnect(byte[] data) => NetManager.DisconnectPeer(this, data);

		public void Disconnect(NetDataWriter writer) => NetManager.DisconnectPeer(this, writer);

		public void Disconnect(byte[] data, int start, int count) => NetManager.DisconnectPeer(this, data, start, count);

		public void Disconnect() => NetManager.DisconnectPeer(this);
		...
		internal void AddReliablePacket(DeliveryMethod method, NetPacket p)
		{
			if (p.IsFragmented)
			{
				...
				//Check for finish
				if (incomingFragments.ReceivedCount != incomingFragments.TotalFragments) return;
		...
		{
			//header + int
			if (packet.Size < NetConstants.PossibleMtu[0]) return;
			...
			{
				//invalid packet
				if (receivedMtu != NetConstants.PossibleMtu[_mtuIdx + 1] - NetManager.ExtraPacketSizeForLayer) return;
		...
		{
			if (_finishMtu) return;

			_mtuCheckTimer += deltaTime;

			if (_mtuCheckTimer < MtuCheckDelay) return;
			...
			lock (_mtuMutex)
			{
				if (_mtuIdx >= NetConstants.PossibleMtu.Length - 1) return;
				...
				if (NetManager.SendRawAndRecycle(p, this) <= 0) _finishMtu = true;
			}
		}
		...
		internal ConnectRequestResult ProcessConnectRequest(NetConnectRequestPacket connRequest)
		{
			...
			switch (_connectionState)
			{
				...
				case ConnectionState.Outgoing:
					...
					if (connRequest.ConnectionTime == _connectTime)
					{
						...
						for (int i = _cachedSocketAddr.Size - 1; i >= 0; i--)
						{
							...
							if (rb == localBytes[i]) continue;

							if (rb < localBytes[i]) return ConnectRequestResult.P2PLose;
						}
					}
				...
				case ConnectionState.ShutdownRequested:
					if (connRequest.ConnectionTime >= _connectTime) return ConnectRequestResult.NewConnection;
		...
		internal virtual void ProcessChanneled(NetPacket packet)
		{
			...
			if (!CreateChannel(packet.ChannelId).ProcessPacket(packet)) NetManager.PoolRecycle(packet);
		}
		...
		internal void SendUserData(NetPacket packet)
		{
			...
			if (_mergePos + mergedPacketSize > _mtu) SendMerged();
		...
		internal void RecycleAndDeliver(NetPacket packet)
		{
			...
			{
				for (int i = 0; i < mergedUserData.Items.Length; i++) NetManager.MessageDelivered(this, mergedUserData.Items[i]);
