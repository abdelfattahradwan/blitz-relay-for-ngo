...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	internal abstract class BaseChannel
	{
		...
		protected BaseChannel(LiteNetPeer peer) => Peer = peer;
		...
		{
			if (Interlocked.CompareExchange(ref _isAddedToPeerChannelSendQueue, 1, 0) == 0) Peer.AddToReliableChannelSendQueue(this);
		}
		...
		public bool SendAndCheckQueue()
		{
			...
			if (!hasPacketsToSend) Interlocked.Exchange(ref _isAddedToPeerChannelSendQueue, 0);
