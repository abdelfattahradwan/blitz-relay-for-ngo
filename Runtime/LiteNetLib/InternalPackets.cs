...
namespace BlitzRelay.Ngo.LiteNetLib
{
	public sealed class NetConnectRequestPacket
	{
		...
		internal static int GetProtocolId(NetPacket packet) => BitConverter.ToInt32(packet.RawData, 1);

		internal static NetConnectRequestPacket FromData(NetPacket packet)
		{
			if (packet.ConnectionNumber >= NetConstants.MaxConnectionNumber) return null;
			...
			if (addrSize != 16 && addrSize != 28) return null;
			...
			if (packet.Size > HeaderSize + addrSize) reader.SetSource(packet.RawData, HeaderSize + addrSize, packet.Size);
		...
		internal static NetPacket Make(ReadOnlySpan<byte> connectData, SocketAddress addressBytes, long connectTime, int localId)
		{
			...
			for (int i = 0; i < addressBytes.Size; i++) packet.RawData[HeaderSize + i] = addressBytes[i];
