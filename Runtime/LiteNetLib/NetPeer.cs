...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public class NetPeer : LiteNetPeer
	{
		...
		/// </exception>
		public void Send(NetDataWriter dataWriter, byte channelNumber, DeliveryMethod deliveryMethod) => SendInternal(dataWriter.AsReadOnlySpan(), channelNumber, deliveryMethod, null);
		...
		///     Fragment count exceeded ushort.MaxValue<para/>
		/// </exception>
		public void Send(byte[] data, byte channelNumber, DeliveryMethod deliveryMethod) => SendInternal(new ReadOnlySpan<byte>(data, 0, data.Length), channelNumber, deliveryMethod, null);
		...
		///     Fragment count exceeded ushort.MaxValue<para/>
		/// </exception>
		public void Send(ReadOnlySpan<byte> data, byte channelNumber, DeliveryMethod deliveryMethod) => SendInternal(data, channelNumber, deliveryMethod, null);
		...
		///     Fragment count exceeded ushort.MaxValue<para/>
		/// </exception>
		public void Send(byte[] data, int start, int length, byte channelNumber, DeliveryMethod deliveryMethod) => SendInternal(new ReadOnlySpan<byte>(data, start, length), channelNumber, deliveryMethod, null);
		...
		/// <param name="ordered">type of channel ReliableOrdered or ReliableUnordered</param>
		/// <returns>packets count in channel queue</returns>
		public new int GetPacketsCountInReliableQueue(bool ordered) => GetPacketsCountInReliableQueue(0, ordered);

		protected override void UpdateChannels()
		{
			//Pending send
			if (_channelSendQueue.IsEmpty) return;

			int count = _channelSendQueue.Count;

			while (count-- > 0)
			{
				if (!_channelSendQueue.TryDequeue(out var channel)) break;
		...
		internal override void ProcessChanneled(NetPacket packet)
		{
			...
			if (channel != null && !channel.ProcessPacket(packet)) NetManager.PoolRecycle(packet);
		}

		internal override void AddToReliableChannelSendQueue(BaseChannel channel) => _channelSendQueue.Enqueue(channel);
		...
		internal override BaseChannel CreateChannel(byte idx)
		{
			...
			if (prevChannel != null) return prevChannel;
