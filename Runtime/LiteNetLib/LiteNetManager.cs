...
using BlitzRelay.Ngo.LiteNetLib.Layers;
using BlitzRelay.Ngo.LiteNetLib.Utils;

namespace BlitzRelay.Ngo.LiteNetLib
{
	/// <summary>
	...
	public partial class LiteNetManager : IEnumerable<LiteNetPeer>
	{
		...
		internal void ConnectionLatencyUpdated(LiteNetPeer fromPeer, int latency) => CreateEvent(NetEvent.EType.ConnectionLatencyUpdated, fromPeer, latency: latency);

		internal void MessageDelivered(LiteNetPeer fromPeer, object userData) => CreateEvent(NetEvent.EType.MessageDelivered, fromPeer, userData: userData);
		...
		(
			...
			NetPacket eventData
		) => DisconnectPeer(peer, reason, socketErrorCode, true, null, 0, 0,
							eventData);
		...
		)
		{
			...
			if (shutdownResult == ShutdownResult.None) return;

			if (shutdownResult == ShutdownResult.WasConnected) Interlocked.Decrement(ref _connectedPeersCount);
		...
		)
		{
			...
			if (type == NetEvent.EType.Connect) Interlocked.Increment(ref _connectedPeersCount);
			else if (type == NetEvent.EType.MessageDelivered) unsyncEvent = UnsyncedDeliveryEvent;

			lock (_eventLock)
			{
				evt = _netEventPoolHead;

				if (evt == null) evt = new NetEvent(this);
				else _netEventPoolHead = evt.Next;
			}
			...
			else
			{
				lock (_eventLock)
				{
					if (_pendingEventTail == null) _pendingEventHead = evt;
					else _pendingEventTail.Next = evt;
		...
		protected virtual void ProcessEvent(NetEvent evt)
		{
			...
			switch (evt.Type)
			{
				...
				case NetEvent.EType.PeerAddressChanged:
					...
					if (previousAddress != null) _netEventListener.OnPeerAddressChanged(evt.Peer, previousAddress);

					break;
			}

			//Recycle if not message
			if (emptyData) RecycleEvent(evt);
			else if (AutoRecycle) evt.DataReader.RecycleInternal();
		}
		...
		private void UpdateLogic()
		{
			...
			while (_isRunning)
			{
				try
				{
					...
					{
						_peersLock.EnterWriteLock();
						for (int i = 0; i < peersToRemove.Count; i++) RemovePeer(peersToRemove[i], false);
						_peersLock.ExitWriteLock();
						peersToRemove.Clear();
					}
					...
					if (sleepTime > 0) _updateTriggerEvent.WaitOne(sleepTime);
				}
		...
		//connect to
		protected virtual LiteNetPeer CreateOutgoingPeer(IPEndPoint remoteEndPoint, int id, byte connectNum, ReadOnlySpan<byte> connectData) => new LiteNetPeer(this, remoteEndPoint, id, connectNum, connectData);

		//accept
		protected virtual LiteNetPeer CreateIncomingPeer(LiteConnectionRequest request, int id) => new LiteNetPeer(this, request, id);

		//reject
		protected virtual LiteNetPeer CreateRejectPeer(IPEndPoint remoteEndPoint, int id) => new LiteNetPeer(this, remoteEndPoint, id);

		protected virtual LiteConnectionRequest CreateConnectionRequest(IPEndPoint remoteEndPoint, NetConnectRequestPacket requestPacket) => new LiteConnectionRequest(remoteEndPoint, requestPacket, this);
		...
		internal LiteNetPeer OnConnectionSolved(LiteConnectionRequest request, byte[] rejectData, int start, int length)
		{
			...
			if (request.Result == ConnectionRequestResult.RejectForce)
			{
				...
				if (rejectData != null && length > 0)
				{
					...
					if (shutdownPacket.Size >= NetConstants.PossibleMtu[0]) NetDebug.WriteError("[Peer] Disconnect additional data size more than MTU!");
					else Buffer.BlockCopy(rejectData, start, shutdownPacket.RawData, 9, length);

					SendRawAndRecycle(shutdownPacket, request.RemoteEndPoint);
				}

				lock (_requestsDict) _requestsDict.Remove(request.RemoteEndPoint);
			}
			else
				lock (_requestsDict)
				{
		...
		private int GetNextPeerId() => _peerIds.TryDequeue(out int id) ? id : _lastPeerId++;
		...
		)
		{
			...
			if (netPeer != null)
			{
				...
				if (processResult != ConnectRequestResult.P2PLose) connRequest.ConnectionNumber = (byte)((netPeer.ConnectionNum + 1) % NetConstants.MaxConnectionNumber);
				//To reconnect peer
			}
			else
			{
		...
		internal virtual bool CustomMessageHandle(NetPacket packet, IPEndPoint remoteEndPoint) => false;
		...
		private void HandleMessageReceived(NetPacket packet, IPEndPoint remoteEndPoint)
		{
			...
			if (CustomMessageHandle(packet, remoteEndPoint)) return;
			...
			switch (packet.Property)
			{
				...
				case PacketProperty.Broadcast:
					if (!BroadcastReceiveEnabled) return;
				...
				case PacketProperty.UnconnectedMessage:
					if (!UnconnectedMessagesEnabled) return;
				...
				case PacketProperty.NatMessage:
					if (NatPunchEnabled) NatPunchModule.ProcessMessage(remoteEndPoint, packet);
			...
			switch (packet.Property)
			{
				case PacketProperty.ConnectRequest:
					...
					if (connRequest != null) ProcessConnectRequest(remoteEndPoint, netPeer, connRequest);
				...
				case PacketProperty.PeerNotFound:
					if (peerFound) //local
					{
						if (netPeer.ConnectionState != ConnectionState.Connected) return;
				...
				case PacketProperty.InvalidProtocol:
					if (peerFound && netPeer.ConnectionState == ConnectionState.Outgoing) DisconnectPeerForce(netPeer, DisconnectReason.InvalidProtocol, 0, null);
				...
				case PacketProperty.ConnectAccept:
					...
					if (connAccept != null && netPeer.ProcessConnectAccept(connAccept)) CreateEvent(NetEvent.EType.Connect, netPeer);

					break;

				default:
					if (peerFound) netPeer.ProcessPacket(packet);
					else SendRawAndRecycle(PoolGetWithProperty(PacketProperty.PeerNotFound), remoteEndPoint);
		...
		public void SendToAll(NetDataWriter writer, DeliveryMethod options) => SendToAll(writer.Data, 0, writer.Length, options);
		...
		/// <param name="options">Send options (reliable, unreliable, etc.)</param>
		public void SendToAll(byte[] data, DeliveryMethod options) => SendToAll(data, 0, data.Length, options);
		...
		public void SendToAll(byte[] data, int start, int length, DeliveryMethod options)
		{
			try
			{
				...
				for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer) netPeer.Send(data, start, length, options);
			}
			finally
			{
		...
		public void SendToAll(NetDataWriter writer, DeliveryMethod options, LiteNetPeer excludePeer) => SendToAll(writer.Data, 0, writer.Length, options, excludePeer);
		...
		/// <param name="options">Send options (reliable, unreliable, etc.)</param>
		/// <param name="excludePeer">Excluded peer</param>
		public void SendToAll(byte[] data, DeliveryMethod options, LiteNetPeer excludePeer) => SendToAll(data, 0, data.Length, options, excludePeer);
		...
		/// <param name="data">Data</param>
		/// <param name="options">Send options (reliable, unreliable, etc.)</param>
		public void SendToAll(ReadOnlySpan<byte> data, DeliveryMethod options) => SendToAll(data, options, null);
		...
		public void SendToAll(ReadOnlySpan<byte> data, DeliveryMethod options, LiteNetPeer excludePeer)
		{
			try
			{
				...
				for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer)
				{
					if (netPeer != excludePeer) netPeer.Send(data, options);
				}
			}
			finally
			{
		...
		public bool Start(IPAddress addressIPv4, IPAddress addressIPv6, int port) => Start(addressIPv4, addressIPv6, port, false);
		...
		/// <param name="addressIPv6">bind to specific ipv6 address</param>
		/// <param name="port">port to listen</param>
		public bool StartInManualMode(IPAddress addressIPv4, IPAddress addressIPv6, int port) => Start(addressIPv4, addressIPv6, port, true);
		...
		/// </summary>
		/// <param name="port">port to listen</param>
		public bool StartInManualMode(int port) => StartInManualMode(IPAddress.Any, IPAddress.IPv6Any, port);
		...
		/// <param name="remoteEndPoint">Packet destination</param>
		/// <returns>Operation result</returns>
		public bool SendUnconnectedMessage(byte[] message, IPEndPoint remoteEndPoint) => SendUnconnectedMessage(message, 0, message.Length, remoteEndPoint);
		...
		/// <returns>Operation result</returns>
		public bool SendUnconnectedMessage(NetDataWriter writer, string address, int port) => SendUnconnectedMessage(writer.Data, 0, writer.Length, NetUtils.MakeEndPoint(address, port));
		...
		/// <param name="remoteEndPoint">Packet destination</param>
		/// <returns>Operation result</returns>
		public bool SendUnconnectedMessage(NetDataWriter writer, IPEndPoint remoteEndPoint) => SendUnconnectedMessage(writer.Data, 0, writer.Length, remoteEndPoint);
		...
		/// </summary>
		public void TriggerUpdate() => _updateTriggerEvent.Set();
		...
		public void PollEvents()
		{
			if (_manualMode)
			{
				if (_udpSocketv4 != null) ManualReceive(_udpSocketv4, _bufferEndPointv4);
				if (_udpSocketv6 != null && _udpSocketv6 != _udpSocketv4) ManualReceive(_udpSocketv6, _bufferEndPointv6);
				ProcessDelayedPackets();
		...
		public LiteNetPeer Connect(string address, int port, string key) => Connect(address, port, NetDataWriter.FromString(key));
		...
		/// <returns>New NetPeer if new connection, Old NetPeer if already connected, null peer if there is ConnectionRequest awaiting</returns>
		/// <exception cref="InvalidOperationException">Manager is not running. Call <see cref="Start()"/></exception>
		public LiteNetPeer Connect(IPEndPoint target, string key) => Connect(target, NetDataWriter.FromString(key));
		...
		public LiteNetPeer Connect(IPEndPoint target, NetDataWriter connectionData)
		{
			...
			lock (_requestsDict)
			{
				if (_requestsDict.ContainsKey(target)) return null;
		...
		public void Stop(bool sendDisconnectMessages)
		{
			...
			for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer) netPeer.Shutdown(null, 0, 0, !sendDisconnectMessages);
			...
			#if DEBUG || SIMULATE_NETWORK
			lock (_pingSimulationList) _pingSimulationList.Clear();
			lock (_outboundSimulationList) _outboundSimulationList.Clear();
			#endif
		...
		public int GetPeersCount(ConnectionState peerState)
		{
			...
			for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer)
			{
				if ((netPeer.ConnectionState & peerState) != 0) count++;
			}
		...
		public void GetPeers(List<LiteNetPeer> peers, ConnectionState peerState)
		{
			...
			for (var netPeer = _headPeer; netPeer != null; netPeer = netPeer.NextPeer)
			{
				if ((netPeer.ConnectionState & peerState) != 0) peers.Add(netPeer);
			}
		...
		/// </summary>
		/// <param name="peers">List that will contain result</param>
		public void GetConnectedPeers(List<LiteNetPeer> peers) => GetPeers(peers, ConnectionState.Connected);

		/// <summary>
		/// Disconnect all peers without any additional data
		/// </summary>
		public void DisconnectAll() => DisconnectAll(null, 0, 0);
		...
		public void DisconnectPeerForce(LiteNetPeer peer) => DisconnectPeerForce(peer, DisconnectReason.DisconnectPeerCalled, 0, null);

		/// <summary>
		/// Disconnect peer from server
		/// </summary>
		/// <param name="peer">peer to disconnect</param>
		public void DisconnectPeer(LiteNetPeer peer) => DisconnectPeer(peer, null, 0, 0);
		...
		public void DisconnectPeer(LiteNetPeer peer, byte[] data) => DisconnectPeer(peer, data, 0, data.Length);
		...
		public void DisconnectPeer(LiteNetPeer peer, NetDataWriter writer) => DisconnectPeer(peer, writer.Data, 0, writer.Length);
		...
		public NetPeerEnumerator<LiteNetPeer> GetEnumerator() => new NetPeerEnumerator<LiteNetPeer>(_headPeer);

		IEnumerator<LiteNetPeer> IEnumerable<LiteNetPeer>.GetEnumerator() => new NetPeerEnumerator<LiteNetPeer>(_headPeer);

		IEnumerator IEnumerable.GetEnumerator() => new NetPeerEnumerator<LiteNetPeer>(_headPeer);
	}
}
