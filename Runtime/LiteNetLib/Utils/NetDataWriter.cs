...
namespace BlitzRelay.Ngo.LiteNetLib.Utils
{
	public class NetDataWriter
	{
		...
		{
			if (_autoResize) ResizeIfNeed(_position + GuidSize);
		...
		{
			if (_autoResize) ResizeIfNeed(_position + data.Length);
		...
		{
			if (_autoResize) ResizeIfNeed(_position + 2 + length);
		...
		public void PutArray(Array arr, int sz)
		{
			...
			sz *= length;
			if (_autoResize) ResizeIfNeed(_position + sz + 2);
			FastBitConverter.GetBytes(_data, _position, length);
			if (arr != null) Buffer.BlockCopy(arr, 0, _data, _position + 2, sz);
			_position += sz + 2;
		}
		...
		public unsafe void PutSpan<T>(Span<T> span) where T : unmanaged
		{
			...
			if (_autoResize) ResizeIfNeed(_position + byteLength + 2);
		...
		public void PutArray(string[] value)
		{
			...
			Put(strArrayLength);
			for (int i = 0; i < strArrayLength; i++) Put(value[i]);
		}
		...
		public void PutArray(string[] value, int strMaxLength)
		{
			ushort strArrayLength = value == null ? (ushort)0 : (ushort)value.Length;
			Put(strArrayLength);
			for (int i = 0; i < strArrayLength; i++) Put(value[i], strMaxLength);
		}
		...
		public void PutArray<T>(T[] value) where T : INetSerializable, new()
		{
			...
			Put(strArrayLength);
			for (int i = 0; i < strArrayLength; i++) value[i].Serialize(this);
		}
		...
		public void Put(IPEndPoint endPoint)
		{
			...
			if (_autoResize) ResizeIfNeed(_position + 1 + addressSize + 2);

			_data[_position++] = familyFlag;

			Span<byte> destination = _data.AsSpan(_position, addressSize);

			if (!endPoint.Address.TryWriteBytes(destination, out int written)) throw new ArgumentException("Failed to write IP bytes.");
		...
		public void Put(string value, int maxLength = 0)
		{
			...
			if (_autoResize) ResizeIfNeed(_position + maxSize + sizeof(ushort));
