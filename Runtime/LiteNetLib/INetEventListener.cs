...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public class EventBasedNetListener : INetEventListener
	{
		...
		void INetEventListener.OnPeerConnected(NetPeer peer) => PeerConnectedEvent?.Invoke(peer);
		...
		void INetEventListener.OnNetworkError(IPEndPoint endPoint, SocketError socketErrorCode) => NetworkErrorEvent?.Invoke(endPoint, socketErrorCode);

		void INetEventListener.OnNetworkReceive(NetPeer peer, NetPacketReader reader, byte channelNumber, DeliveryMethod deliveryMethod) => NetworkReceiveEvent?.Invoke(peer, reader, channelNumber, deliveryMethod);
		...
		void INetEventListener.OnMessageDelivered(NetPeer peer, object userData) => DeliveryEvent?.Invoke(peer, userData);

		void INetEventListener.OnNtpResponse(NtpPacket packet) => NtpResponseEvent?.Invoke(packet);

		void INetEventListener.OnPeerAddressChanged(NetPeer peer, IPEndPoint previousAddress) => PeerAddressChangedEvent?.Invoke(peer, previousAddress);
	}
	...
	public class EventBasedLiteNetListener : ILiteNetEventListener
	{
		...
		void ILiteNetEventListener.OnPeerConnected(LiteNetPeer peer) => PeerConnectedEvent?.Invoke(peer);

		void ILiteNetEventListener.OnPeerDisconnected(LiteNetPeer peer, DisconnectInfo disconnectInfo) => PeerDisconnectedEvent?.Invoke(peer, disconnectInfo);

		void ILiteNetEventListener.OnNetworkError(IPEndPoint endPoint, SocketError socketErrorCode) => NetworkErrorEvent?.Invoke(endPoint, socketErrorCode);

		void ILiteNetEventListener.OnNetworkReceive(LiteNetPeer peer, NetPacketReader reader, DeliveryMethod deliveryMethod) => NetworkReceiveEvent?.Invoke(peer, reader, deliveryMethod);

		void ILiteNetEventListener.OnNetworkReceiveUnconnected(IPEndPoint remoteEndPoint, NetPacketReader reader, UnconnectedMessageType messageType) => NetworkReceiveUnconnectedEvent?.Invoke(remoteEndPoint, reader, messageType);

		void ILiteNetEventListener.OnNetworkLatencyUpdate(LiteNetPeer peer, int latency) => NetworkLatencyUpdateEvent?.Invoke(peer, latency);

		void ILiteNetEventListener.OnConnectionRequest(LiteConnectionRequest request) => ConnectionRequestEvent?.Invoke(request);

		void ILiteNetEventListener.OnMessageDelivered(LiteNetPeer peer, object userData) => DeliveryEvent?.Invoke(peer, userData);

		void ILiteNetEventListener.OnPeerAddressChanged(LiteNetPeer peer, IPEndPoint previousAddress) => PeerAddressChangedEvent?.Invoke(peer, previousAddress);
	}
}
