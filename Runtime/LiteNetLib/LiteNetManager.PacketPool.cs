...
namespace BlitzRelay.Ngo.LiteNetLib
{
	public partial class LiteNetManager
	{
		...
		{
			if (size > NetConstants.MaxPacketSize) return new NetPacket(size);
			...
			lock (_poolLock)
			{
				...
				if (packet == null) return new NetPacket(size);
			...
			packet.Size = size;
			if (packet.RawData.Length < size) packet.RawData = new byte[size];
