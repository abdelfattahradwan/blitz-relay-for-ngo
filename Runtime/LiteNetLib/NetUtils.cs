...
namespace BlitzRelay.Ngo.LiteNetLib
{
	...
	public static class NetUtils
	{
		...
		public static IPEndPoint MakeEndPoint(string hostStr, int port) => new IPEndPoint(ResolveAddress(hostStr), port);
		...
		{
			if (hostStr == "localhost") return IPAddress.Loopback;

			if (!IPAddress.TryParse(hostStr, out var ipAddress))
			{
				if (LiteNetManager.IPv6Support) ipAddress = ResolveAddress(hostStr, AddressFamily.InterNetworkV6);
				if (ipAddress == null) ipAddress = ResolveAddress(hostStr, AddressFamily.InterNetwork);
			}

			if (ipAddress == null) throw new ArgumentException("Invalid address: " + hostStr);
		...
		public static void GetLocalIpList(IList<string> targetList, LocalAddrType addrType)
		{
				...
				foreach (NetworkInterface ni in networks)
				{
					...
					foreach (UnicastIPAddressInformation ip in ipProps.UnicastAddresses)
					{
						...
						if ((ipv4 && address.AddressFamily == AddressFamily.InterNetwork) ||
							(ipv6 && address.AddressFamily == AddressFamily.InterNetworkV6)) targetList.Add(address.ToString());
					}
				}
				...
				if (targetList.Count == 0)
				{
					...
					foreach (IPAddress ip in addresses)
					{
						if ((ipv4 && ip.AddressFamily == AddressFamily.InterNetwork) ||
							(ipv6 && ip.AddressFamily == AddressFamily.InterNetworkV6)) targetList.Add(ip.ToString());
					}
				}
			}
			catch
			...
			if (targetList.Count == 0)
			{
				if (ipv4) targetList.Add("127.0.0.1");
				if (ipv6) targetList.Add("::1");
			}
		}
		...
		internal static int RelativeSequenceNumber(int number, int expected) => (number - expected + NetConstants.MaxSequence + NetConstants.HalfMaxSequence) % NetConstants.MaxSequence - NetConstants.HalfMaxSequence;
	...
	internal class NetworkSorter : IComparer<NetworkInterface>
	{
		...
		public int Compare(NetworkInterface a, NetworkInterface b)
		{
			...
			var priorityA = isEthernetA ? 3 :
							isWifiA     ? 2 :
							isOtherA    ? 1 : 0;

			var priorityB = isEthernetB ? 3 :
							isWifiB     ? 2 :
							isOtherB    ? 1 : 0;
