	...
	public partial class LiteNetManager
	{
		...
		private void ManualReceive(Socket socket, EndPoint bufferEndPoint)
		{
			...
			try
			{
				...
				{
					if (ReceiveFrom(socket, ref bufferEndPoint) > 0) count--;
					else break;
				}
			}
		...
		private void NativeReceiveLogic()
		{
			...
			while (_isRunning)
			{
				try
				{
					if (socketV6 == null)
					{
						if (NativeReceiveFrom(socketHandle4, addrBuffer4) == false) return;
					...
					{
						if (NativeReceiveFrom(socketHandle6, addrBuffer6) == false) return;
			...
			bool NativeReceiveFrom(IntPtr s, byte[] address)
			{
				...
				if (packet.Size == 0) return true; //socket closed or empty packet
		...
		private void ReceiveLogic()
		{
			...
			while (_isRunning)
			{
				...
				try
				{
					if (socketV6 == null)
					{
						if (socketv4.Available == 0 && !socketv4.Poll(ReceivePollingTime, SelectMode.SelectRead)) continue;
		...
		{
			if (IsRunning && NotConnected == false) return false;
			...
			#if UNITY_SOCKET_FIX
			if (_useSocketFix && _pausedSocketFix == null) _pausedSocketFix = new PausedSocketFix(this, addressIPv4, addressIPv6, port, manualMode);
			#endif
			...
			if (IPv6Support && IPv6Enabled)
			{
				...
				{
					if (_manualMode) _bufferEndPointv6 = new IPEndPoint(IPAddress.IPv6Any, 0);
				}
				else
				{
					_udpSocketv6 = null;
			...
			if (!manualMode)
			{
				...
				if (UseNativeSockets) ts = NativeReceiveLogic;
		...
		internal int SendRaw(NetPacket packet, IPEndPoint remoteEndPoint) => SendRaw(packet.RawData, 0, packet.Size, remoteEndPoint);
		...
		internal int SendRaw(byte[] message, int start, int length, IPEndPoint remoteEndPoint)
		{
			...
			{
				if (expandedPacket != null) PoolRecycle(expandedPacket);
		...
		internal int SendRawCore(byte[] message, int start, int length, IPEndPoint remoteEndPoint)
		{
			...
			try
			{
				if (UseNativeSockets && remoteEndPoint is LiteNetPeer peer)
				{
					unsafe
					{
						fixed (byte* dataWithOffset = &message[start]) result = NativeSocket.SendTo(socket.Handle, dataWithOffset, length, peer.NativeAddress, peer.NativeAddress.Length);
					}

					if (result == -1) throw NativeSocket.GetSocketException();
				}
				else
				{
					#if NET8_0_OR_GREATER
		...
		public bool SendBroadcast(NetDataWriter writer, int port) => SendBroadcast(writer.Data, 0, writer.Length, port);

		public bool SendBroadcast(byte[] data, int port) => SendBroadcast(data, 0, data.Length, port);
		...
		public bool SendBroadcast(byte[] data, int start, int length, int port)
		{
			...
			{
				if (ex.SocketErrorCode == SocketError.HostUnreachable) return broadcastSuccess;
		...
		{
			_isRunning = false;
			if (_receiveThread != null && _receiveThread != Thread.CurrentThread) _receiveThread.Join();
			_receiveThread = null;
			_udpSocketv4?.Close();
			_udpSocketv6?.Close();
			_udpSocketv4 = null;
